...
const {
  sendCreateSocietyEmail,
  sendResetPasswordEmail,
} = require("../../utilities/email/sendEmails");
...
module.exports.society_signup = async (req, res) => {
  ...
  try {
    ...
    fs.rename(
      `upload/society_logos/temp.jpeg`,
      `upload/society_logos/${society_id}_${originalName}`,
      (err) => {
        if (err) {
    ...
    const modifiedSocieties = await Promise.all(
      societies.map(async (society) => {
        ...
        if (application_data != null) {
          const newModifedSociety = {
            ...modifiedSociety,
            certificate: application_data.certificate,
          };
...
module.exports.checkOtp = async (req, res) => {
  try {
    ...
    const otp_generation = new Date(
      society.otp_generation_time.getTime() + 5 * 60 * 1000
    );
